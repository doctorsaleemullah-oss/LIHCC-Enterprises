import { saveCustomer } from "@/actions/masters";
...
export default async function NewCustomerPage() {
  await requireAccess("customers", "create");
  async function action(formData: FormData) {
    "use server";
    const r = await saveCustomer(formData);
  ...
  return (
    <div className="max-w-3xl">
      ...
      <form action={action} className="card p-6 grid md:grid-cols-2 gap-4">
        <div className="md:col-span-2"><label className="label">Name *</label><input className="input" name="name" required /></div>
        <div><label className="label">Organization</label><input className="input" name="organization" /></div>
        ...
        <div><label className="label">Opening receivable</label><input className="input" name="openingBalance" type="number" step="0.01" defaultValue={0} /></div>
        <div><label className="label">Opening as of date</label><input className="input" name="transactionDate" type="date" /></div>
        <div className="md:col-span-2 flex items-center gap-2 text-sm">
          <input type="checkbox" name="isDedicatedLihcc" /> Luqman Cardiac Center dedicated account
        </div>
